...
class Gridworld(mdp.MarkovDecisionProcess):
    ...
    def getTransitionStatesAndProbs(self, state, action):
        ...
        if action not in self.getPossibleActions(state):
            raise Exception(f"Illegal action, {action} at state {state}!")
...
class GridworldEnvironment(environment.Environment):
    ...
    def __init__(self, gridWorld, display, pause):
...
def runManual(environment, discount, message, episode):
    returns = 0
    totalDiscount = 1.0
    environment.reset()
    ...
    while True:
        ...
        action = getUserAction(state, environment.getPossibleActions)
        if action == None:
